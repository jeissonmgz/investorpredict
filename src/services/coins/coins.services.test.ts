...
it('should get coins value time', async () =>{
    ...
    expect(getSpy).toBeCalledWith(`url/coins//market_chart?vs_currency=usd&days=90&interval=daily`);
    expect(coins).toEqual({});
})
it('should get coins details', async () =>{
    getSpy.mockResolvedValue({});
    const coins = await CoinService.getCoinDetail('');
    expect(getSpy).toBeCalledWith(`url/coins/?localization=true&tickers=false&market_data=false&community_data=false&developer_data=false&sparkline=false`);
    expect(coins).toEqual({});
})
